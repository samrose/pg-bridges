...
CREATE SCHEMA IF NOT EXISTS pg_elixir;

-- Grant usage on schema
GRANT USAGE ON SCHEMA pg_elixir TO PUBLIC;

-- Create a table to store async results (optional, for persistence)
CREATE TABLE IF NOT EXISTS pg_elixir.async_results (
...
CREATE INDEX IF NOT EXISTS idx_async_results_status
    ON pg_elixir.async_results(status);

CREATE INDEX IF NOT EXISTS idx_async_results_created
    ON pg_elixir.async_results(created_at);

-- Create a table for function metrics
CREATE TABLE IF NOT EXISTS pg_elixir.function_metrics (
...
CREATE OR REPLACE FUNCTION pg_elixir.cleanup_old_results()
RETURNS INTEGER AS $$
DECLARE
    deleted_count INTEGER;
BEGIN
    DELETE FROM pg_elixir.async_results
...
CREATE OR REPLACE FUNCTION pg_elixir.safe_call(
    function_name TEXT,
    args JSONB,
...
BEGIN
    ...
    -- Update metrics
    INSERT INTO pg_elixir.function_metrics (
        function_name,
        call_count,
        total_duration_ms,
        last_called
    ...
    ON CONFLICT (function_name) DO UPDATE SET
        call_count = pg_elixir.function_metrics.call_count + 1,
        total_duration_ms = pg_elixir.function_metrics.total_duration_ms + duration_ms,
        last_called = NOW();
...
EXCEPTION WHEN OTHERS THEN
    ...
    ON CONFLICT (function_name) DO UPDATE SET
        error_count = pg_elixir.function_metrics.error_count + 1,
        last_error = SQLERRM,
        last_called = NOW();
...
CREATE OR REPLACE VIEW pg_elixir.function_stats AS
SELECT
    function_name,
    call_count,
    error_count,
    ...
    last_called,
    last_error
FROM pg_elixir.function_metrics
ORDER BY call_count DESC;

-- Grant permissions
GRANT SELECT ON pg_elixir.function_stats TO PUBLIC;
GRANT SELECT, INSERT, UPDATE ON pg_elixir.async_results TO PUBLIC;
GRANT SELECT, INSERT, UPDATE ON pg_elixir.function_metrics TO PUBLIC;
